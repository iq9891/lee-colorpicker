...
function bindDom(self, parmas) {
  ...
  self.direction = parmas.direction;
...
function dragPos($obj, direction, top, left) {
  if (direction === 'any') {
    $obj.css({
      top,
      left,
...
function mousemoveHandler(self, e) {
  ...
  // 如果不让拖动的元素动
  if (self.bIdDrag) {
    dragPos(self.$id, self.direction, newTop, newLeft);
  }
  /**
   * @return
   * e 事件对象
...
/**
  ...
  * direction : 拖拽方向，默认是 any(任意) row(横) col(竖)
